...
Software development, Communication, Project management
\end{IEEEkeywords}
...
\subsection{Elliptic curve Cryptosystem (ECC)}
...
We found that STEP \ref{itm:step6} of algorithm of Section \ref{sec:secureMrea} can be further enhanced by third prime number. STEP \ref{itm:step6} can be written as: $e \times d \mod \phi = 1$, \emph{i.e}
\begin{equation}
e \times d = 1 + x(\phi)
\end{equation}
...
MT19937 has to following statistical property which makes it preferable over other pseudorandom number generators.
\begin{itemize}
...
In Miller-Rabin primality test the number to be tested say $p$ and $y$ be a non trivial square root of $1 (\mod p)$. Then we must have that $y^{2} = 1 (\mod p)$ and so $(y-1)(y+1) = 0(\mod p)$. This implies that either $y = 1 \mod p$ or $y = -1 \mod p$, which implies that $y$ is a trivial square root.
...
%Though GMPY2 is suitable for faster computation but it is limited to python's exponential computation. Karatsuba Multiplication algorithm mentioned in section \ref{sec: karatsuba_code} was implemented to enhance the performance of multiplication but showed no significant performance. The paper\cite{securersa} shows algorithm using 32 bit keys to generate prime numbers but this algorithm works to generate keys of size unto 512 bit keys and showed significant performance.
